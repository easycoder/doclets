...
// The main entry point is Webson.render(), which takes a container element,
...
// for training purposes.
    ...
    reportTiming: (message) => {
        if (!Webson.timingEnabled) {
            return;
        }
        if (typeof Webson.timingReporter === `function`) {
            Webson.timingReporter(message);
        } else {
            console.log(message);
        }
    },
    ...
    build: async (parent, name, items, parentSymbols) => {
        const buildStartedAt = Webson.nowMs();
        ...
        const waitStartedAt = Webson.nowMs();
        await Webson.waitForElementReady(element);
        const waitElapsed = Math.round(Webson.nowMs() - waitStartedAt);
        const totalElapsed = Math.round(Webson.nowMs() - buildStartedAt);
        const tag = element && element.tagName ? element.tagName.toLowerCase() : `unknown`;
        Webson.reportTiming(`[WebsonTiming] name='${name}' tag='${tag}' wait=${waitElapsed}ms total=${totalElapsed}ms`);
    },
    ...
    render: async (parent, name, script, options = {}) => {
        ...
        Webson.name = name;
        Webson.script = typeof script === `string` ? JSON.parse(script) : script;
        Webson.timingEnabled = !!options.timingEnabled;
        Webson.timingReporter = typeof options.timingReporter === `function`
...
if (typeof globalThis !== `undefined`) {
    globalThis.Webson = Webson;
}

if (typeof module !== `undefined` && module.exports) {
    module.exports = Webson;
}
